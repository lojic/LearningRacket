...
;; In the 20×20 grid below, four numbers along a diagonal line have been marked identified.
;; (Row,Col) Number
;; ( 7, 9) 26
;; ( 8,10) 63
;; ( 9,11) 78
...
(define grid 
  (vector (vector 08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08)
...
(define (elem grid row col)
  (vector-ref (vector-ref grid row) col))

(define (four-prod grid row row-delta col col-delta)
  (* (elem grid row col)
     (elem grid (+ row row-delta) (+ col col-delta))
     (elem grid (+ row (* 2 row-delta)) (+ col (* 2 col-delta)))
     (elem grid (+ row (* 3 row-delta)) (+ col (* 3 col-delta)))))

(define (horizontal grid row col)
  (four-prod grid row 0 col 1))

(define (diagonal1 grid row col)
  (four-prod grid row 1 col 1))

(define (diagonal2 grid row col)
  (four-prod grid row 1 col -1))

(define (vertical grid row col)
  (four-prod grid row 1 col 0))

(define (max-func f grid row-range col-range)
  (let ([max 0])
    (for ([row row-range])
      (for ([col col-range])
        (let ([prod (f grid row col)])
          (when (> prod max)
            (set! max prod)))))
    max))

(max
  (max-func horizontal grid grid-dim (- grid-dim 3))
  (max-func diagonal1 grid (- grid-dim 3) (- grid-dim 3))
  (max-func diagonal2 grid (- grid-dim 3) (in-range 3 grid-dim))
  (max-func vertical grid (- grid-dim 3) grid-dim))
