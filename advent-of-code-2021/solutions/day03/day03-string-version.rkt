...
(define (flip s) (build-string (string-length s) (λ (i) (flipc (string-ref s i)))))
(define (gamma lst) (build-string (string-length (car lst)) (λ (i) (common (col lst i)))))
(define (part1 input) (let ([ g (gamma input) ]) (* (to-decimal g) (to-decimal (flip g)))))
(define (part2 input)
  (define (life input oxy)
    (car (let loop ([ lst input ][ bit 0 ])
           (if (or (>= bit (string-length (car input))) (< (length lst) 2))
               lst
